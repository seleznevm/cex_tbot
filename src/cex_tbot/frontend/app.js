...
const state = { activeView: 'dashboard', proposals: [], selectedProposalId: null, dashboard: null, noTrades: [], refreshTimer: null, loading: false, reportMode: localStorage.getItem(STORAGE_KEYS.reportMode) || 'text', currentPage: 1 };
const els = {
...
function paginate(items) { const pageSize = Number(els.proposalPageSize.value || 10); const totalPages = Math.max(1, Math.ceil(items.length / pageSize)); if (state.currentPage > totalPages) state.currentPage = totalPages; const start = (state.currentPage - 1) * pageSize; return { pageItems: items.slice(start, start + pageSize), totalPages, pageSize }; }
...
async function refreshAll(showLoading = true) { if (showLoading) setLoading(true, 'Refreshing…'); try { const [dashboard, proposals, noTrades] = await Promise.all([apiFetch('/dashboard'), apiFetch('/proposals'), apiFetch('/no-trades')]); state.dashboard = dashboard; state.proposals = proposals; state.noTrades = noTrades; populateStatusFilter(); renderProposalSubmitForm(); renderDashboard(); renderProposalList(); renderNoTrades(); setStatus(`Updated at ${new Date().toLocaleTimeString()}`); } catch (error) { setStatus('Refresh failed'); toast(error.message, true); } finally { if (showLoading) setLoading(false); } }
