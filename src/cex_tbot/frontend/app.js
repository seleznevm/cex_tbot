...
function renderDashboard() {
  ...
  els.dashboardView.innerHTML = `
    <div class="cards">
      ...
      <div class="card"><h3>Safety state</h3><div class="metric ${d.risk.block_new_trades || d.risk.emergency_halt_active ? 'danger-text' : 'success-text'}">${d.risk.safety_state}</div></div>
    </div>
    <div class="panel nested-panel"><div class="detail-header"><div><h3>System controls</h3><p class="muted">Emergency halt for operator flows.</p></div></div><label class="field"><span>Halt reason</span><input id="halt-reason" type="text" value="manual-safety-stop" /></label><div class="actions wrap"><button id="halt-btn" class="danger">Halt</button><button id="unhalt-btn">Unhalt</button><button id="clear-safety-btn">Clear safety</button>${d.risk.halt_reason ? `<span class="muted">Current reason: ${d.risk.halt_reason}</span>` : ''}${d.risk.block_reason ? `<span class="muted">Block reason: ${d.risk.block_reason}</span>` : ''}</div></div>
    <div class="panel nested-panel"><h3>Risk budget</h3><div class="detail-grid"><div><span class="muted">Safety state</span><div>${d.risk.safety_state}</div></div><div><span class="muted">Block new trades</span><div>${d.risk.block_new_trades ? 'YES' : 'NO'}</div></div><div><span class="muted">Max open risk</span><div>${d.risk.max_open_risk_percent}%</div></div><div><span class="muted">Active risk</span><div>${d.risk.active_risk_percent}%</div></div><div><span class="muted">Reserved pending</span><div>${d.risk.reserved_pending_risk_percent}%</div></div><div><span class="muted">Free budget</span><div>${d.risk.free_risk_budget_percent}%</div></div></div></div>
    ...
    <div class="panel nested-panel"><h3>Alerts</h3>${d.alerts?.items?.length ? d.alerts.items.map(item => `<div class="proposal-item"><strong>${item.code}</strong><br><span class="muted">${item.level}</span><br>${item.message}</div>`).join('') : '<p class="muted">No active alerts.</p>'}</div>
    <div class="panel nested-panel"><h3>Operator activity</h3>${d.operator_activity?.recent_items?.length ? d.operator_activity.recent_items.map(item => `<div class="proposal-item ${item.proposal_id ? 'quick-open' : ''}" ${item.proposal_id ? `data-proposal-id="${item.proposal_id}"` : ''}><strong>${item.outcome}</strong><br><span class="muted">${item.actor} · ${new Date(item.created_at).toLocaleString()}</span><br><span>${item.raw_command}</span>${item.proposal_id ? `<br><span class="muted">${item.proposal_id}</span>` : ''}</div>`).join('') : '<p class="muted">No operator activity yet.</p>'}</div>
    <div class="panel nested-panel"><h3>Universe status</h3><div class="detail-grid"><div><span class="muted">Snapshot</span><div>${d.universe?.snapshot_id || '—'}</div></div><div><span class="muted">Refresh reason</span><div>${d.universe?.refresh_reason || '—'}</div></div><div><span class="muted">Last refresh</span><div>${d.universe?.last_refresh_at ? new Date(d.universe.last_refresh_at).toLocaleString() : '—'}</div></div><div><span class="muted">Total instruments</span><div>${d.universe?.total_instruments ?? 0}</div></div><div><span class="muted">Eligible</span><div>${d.universe?.eligible_instruments ?? 0}</div></div><div><span class="muted">Ineligible</span><div>${d.universe?.ineligible_instruments ?? 0}</div></div><div><span class="muted">Stale</span><div>${d.universe?.stale_instruments ?? 0}</div></div></div>${d.universe?.eligible_symbols?.length ? `<div class="nested-panel"><span class="muted">Eligible symbols</span><div class="meta-row" style="flex-wrap:wrap; margin-top:8px;">${d.universe.eligible_symbols.map(symbol => `<span class="badge neutral">${symbol}</span>`).join('')}</div></div>` : '<p class="muted">No eligible symbols yet.</p>'}</div>
    <div class="panel nested-panel"><h3>Post-analysis</h3><div class="detail-grid"><div><span class="muted">Total trades</span><div>${d.post_analysis?.total_trades ?? 0}</div></div><div><span class="muted">Executed</span><div>${d.post_analysis?.executed_trades ?? 0}</div></div><div><span class="muted">No-trades</span><div>${d.post_analysis?.no_trade_decisions ?? 0}</div></div><div><span class="muted">Avg confidence</span><div>${d.post_analysis?.avg_confidence_all ?? 0}</div></div><div><span class="muted">Recent trades</span><div>${d.post_analysis?.recent_trade_count ?? 0}</div></div><div><span class="muted">Recent executed</span><div>${d.post_analysis?.recent_executed_trades ?? 0}</div></div><div><span class="muted">Recent rejected</span><div>${d.post_analysis?.recent_rejected_trades ?? 0}</div></div><div><span class="muted">Recent avg conf</span><div>${d.post_analysis?.recent_avg_confidence ?? 0}</div></div><div><span class="muted">Top strategy</span><div>${d.post_analysis?.top_strategy || '—'}</div></div><div><span class="muted">Top timeframe</span><div>${d.post_analysis?.top_timeframe || '—'}</div></div><div><span class="muted">Top reject</span><div>${d.post_analysis?.top_rejection_status || '—'}</div></div><div><span class="muted">Top no-trade</span><div>${d.post_analysis?.top_no_trade_reason || '—'}</div></div></div>${d.post_analysis?.trend_hint ? `<div class="nested-panel"><span class="muted">Recent vs all-time</span><div style="margin-top:8px;">${d.post_analysis.trend_hint}</div></div>` : ''}${d.post_analysis?.recommendations?.length ? `<div class="nested-panel"><span class="muted">Recommendations</span><div style="margin-top:8px;">${d.post_analysis.recommendations.map(item => `<div class="proposal-item"><strong>[${item.severity}] ${item.action}</strong><br><span class="muted">${item.target}</span><br>${item.reason}</div>`).join('')}</div></div>` : ''}${d.post_analysis?.latest_hint ? `<div class="nested-panel"><span class="muted">Latest calibration hint</span><div style="margin-top:8px;">${d.post_analysis.latest_hint}</div></div>` : '<p class="muted">No calibration hint yet.</p>'}</div>
