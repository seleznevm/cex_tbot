...
function renderDashboard() {
  ...
  els.dashboardView.innerHTML = `
    ...
    <div class="panel nested-panel"><h3>Post-analysis</h3><div class="detail-grid"><div><span class="muted">Total trades</span><div>${d.post_analysis?.total_trades ?? 0}</div></div><div><span class="muted">Executed</span><div>${d.post_analysis?.executed_trades ?? 0}</div></div><div><span class="muted">No-trades</span><div>${d.post_analysis?.no_trade_decisions ?? 0}</div></div><div><span class="muted">Avg confidence</span><div>${d.post_analysis?.avg_confidence_all ?? 0}</div></div><div><span class="muted">Recent trades</span><div>${d.post_analysis?.recent_trade_count ?? 0}</div></div><div><span class="muted">Recent avg conf</span><div>${d.post_analysis?.recent_avg_confidence ?? 0}</div></div><div><span class="muted">Top strategy</span><div>${d.post_analysis?.top_strategy || '—'}</div></div><div><span class="muted">Top timeframe</span><div>${d.post_analysis?.top_timeframe || '—'}</div></div><div><span class="muted">Top reject</span><div>${d.post_analysis?.top_rejection_status || '—'}</div></div><div><span class="muted">Top no-trade</span><div>${d.post_analysis?.top_no_trade_reason || '—'}</div></div></div>${d.post_analysis?.latest_hint ? `<div class="nested-panel"><span class="muted">Latest calibration hint</span><div style="margin-top:8px;">${d.post_analysis.latest_hint}</div></div>` : '<p class="muted">No calibration hint yet.</p>'}</div>
