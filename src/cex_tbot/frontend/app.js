...
function renderDashboard() {
  ...
  els.dashboardView.innerHTML = `
    ...
    <div class="panel nested-panel"><div class="detail-header"><div><h3>System controls</h3><p class="muted">Emergency halt for operator flows.</p></div></div><label class="field"><span>Halt reason</span><input id="halt-reason" type="text" value="manual-safety-stop" /></label><div class="actions wrap"><button id="halt-btn" class="danger">Halt</button><button id="unhalt-btn">Unhalt</button>${d.risk.halt_reason ? `<span class="muted">Current reason: ${d.risk.halt_reason}</span>` : ''}${d.risk.block_reason ? `<span class="muted">Block reason: ${d.risk.block_reason}</span>` : ''}</div></div>
