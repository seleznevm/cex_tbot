...
class FileTradeSessionStoreTests(unittest.TestCase):
    def test_reloads_full_session_components(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            session = FileTradeSessionStore.open(Path(tmp) / "session")
            ...
            reloaded = FileTradeSessionStore.open(Path(tmp) / "session")
            self.assertEqual(reloaded.proposals.get("proposal_1").status, ProposalStatus.EXECUTED)
            self.assertGreaterEqual(len(reloaded.execution_journal.list_events("proposal_1")), 3)
            self.assertGreaterEqual(len(reloaded.execution_state.list_snapshots("proposal_1")), 2)
            self.assertEqual(len(reloaded.operator_transcript.list_entries("proposal_1")), 1)
