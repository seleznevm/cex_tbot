...
class OperatorTranscriptTests(unittest.TestCase):
    def test_records_operator_command_outcomes(self) -> None:
        ...
        router = OperatorCommandRouter(workflow, approval, transcript=transcript)
        router.route("Mike", "APPROVE proposal_1", PortfolioState(equity=1000.0), now=now)
        entries = transcript.list_entries("proposal_1")
        self.assertEqual(len(entries), 1)
        self.assertEqual(entries[0].outcome, "APPROVE")
